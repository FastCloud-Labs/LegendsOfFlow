<template>
  <v-layout class="rounded rounded-md fill-height">
    <v-app-bar>
      ...
      <v-menu>
        <template v-slot:activator="{ props }">
          <v-btn v-bind="props" size="small" class="ma-0 pa-0 mr-2">
            <v-icon icon="fas fa-cog" />
          </v-btn>
        </template>
        <v-list>
          <v-list-item v-if="!user.profile?.dapperAddress">
            <v-list-item-title @click="showDapper()"
              >Connect Dapper Wallet</v-list-item-title
            >
          </v-list-item>
    ...
    <div
      class="deskop-menu hidden-sm-and-down fill-height"
      width="200"
      height="100%"
    >
      <v-list>
    ...
    <v-navigation-drawer
      ...
      location="bottom"
      temporary
      class="mobile-menu hidden-md-and-up"
    >
      <v-list>
        <v-list-item @click="showMyEvents">
          <v-icon
    ...
    <v-main class="d-flex justify-center" :width="width">
      <div v-if="showDapperConnect">
        <Dapper
          :user="user"
          @showUpcomingGames="showUpcomingEvents"
  ...
  methods: {
    checkUser(user) {
      ...
      if (!this.profile?.avatar) {
        this.profile.avatar = `https://source.boringavatars.com/pixel/60/${
